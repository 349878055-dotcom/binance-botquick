...
#include <mutex> // For std::mutex
    ...
    void write_order_event(const char* client_order_id, const char* parent_order_id, const char* exch_order_id, int event_type, 
    ...
    void on_order_update_private_stream(const char* client_order_id, const char* parent_order_id, const char* exch_order_id, 
        ...
        if (it != g_active_orders.end()) {
            ...
            write_order_event(client_order_id, parent_order_id, exch_order_id, order_status_event, fill_px, fill_qty, remaining_qty, 0, "", current_update_id, is_maker, transact_time, trigger_ms);
            ...
            write_order_event(client_order_id, parent_order_id, exch_order_id, order_status_event, fill_px, fill_qty, remaining_qty, 0, "Unknown order event from exchange.", current_update_id, is_maker, transact_time, trigger_ms);
        }
    }

    // 原始 on_order_filled 函数现在用于接收来自 WebSocket 的原始订单事件（例如来自 User Data Stream）
    // 并且会调用 on_order_update_private_stream 进行处理和去重。
    void on_order_filled(const char* client_order_id, const char* parent_order_id, const char* exch_order_id, double fill_px, double fill_qty, double remaining_qty, int order_status_event, uint64_t transact_time, uint64_t last_update_id, bool is_maker, uint64_t trigger_ms) {
        on_order_update_private_stream(client_order_id, parent_order_id, exch_order_id, fill_px, fill_qty, remaining_qty, order_status_event, last_update_id, is_maker, transact_time, trigger_ms);
    }
