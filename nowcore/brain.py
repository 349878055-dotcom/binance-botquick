...
class MomentumMonitor:
    ...
    def __init__(self):
        ...
        self.history = deque(maxlen=2000) # 限制 history 队列的长度，例如 2000 条
...
class NeuroCore:
    ...
    def execute_flip(self, target_side_int: int, source: str): # 增加 source 参数
        ...
        if self.last_trade_price <= 0:
            ...
            self.trigger_log.append({"timestamp": get_now_ns(), "momentum": 0.0, "action": "NO_TRADE_PRICE_ZERO", "source": source}) # 记录触发源
        ...
        if old_logic_side != 0: # 如果当前有持仓，先平仓
            ...
            print(f"[FLIP] 触发源: {source} | 动量反转！立刻平仓 {physical_pos} 并开仓 {target_side_str}") # 加入触发源信息
            ...
            self.trigger_log.append((get_now_ns(), 0.0, -old_logic_side, self.last_trade_price, source)) # 记录平仓动作及触发源
        ...
        self.trigger_log.append((get_now_ns(), 0.0, target_side_int, self.last_trade_price, source)) # 记录开仓动作及触发源
