...
class Genes:
    ...
    ACCUM_1MIN_THRESHOLD = 80.0 # 累计净量：1分钟内单向累积 > 80 BTC
    ACCUM_1MIN_THRESHOLD = 80.0 # 累计净量：1分钟内单向累积 > 80 BTC
...
class MomentumMonitor:
    ...
    def __init__(self):
        self.history = deque(maxlen=20000) # 限制 history 队列的长度，例如 20000 条
    ...
    def get_collapse_metrics(self) -> tuple[float, float, float, int]:
        ...
        current_time = self.get_current_physical_timestamp_ns()
...
class NeuroCore:
    ...
    def __init__(self):
        ...
        self.get_current_physical_timestamp_ns = lambda: self.last_market_frame_timestamp
    ...
    def _fire_atomic_order(self, side: str, qty: float, order_type_str: str, reason: str, tif_type: int = 3, aggressive_slippage_override: float = None) -> str:
        ...
        self.last_order_trigger_ms = self.get_current_physical_timestamp_ns() # 使用 Common.get_now_ns() 获取纳秒时间戳
    ...
    def logic_loop(self):
        ...
        trigger_B = (abs(v_net_1min) > Genes.ACCUM_1MIN_THRESHOLD) # 使用 Genes 中的阈值
        ...
        if trigger_A or trigger_B:
            ...
            if trigger_A:
                ...
                trigger_source = "PULSE_25MS"
            elif trigger_B:
                target_side_int = 1 if v_net_1min > 0 else -1
                trigger_source = "ACCUM_1MIN"
            
            if target_side_int != 0 and target_side_int != self.logic_side:
                # 执行 150U 翻转连发协议
                self.execute_flip(target_side_int, trigger_source) # 传入触发源

    def execute_flip(self, target_side_int: int, source: str):
        """
        清理后的 Flip 逻辑：一次只做一个物理动作。
        """
        physical_pos = self.bridge.data.account_feed.position_amt
        usdt_bal = self.bridge.data.account_feed.usdt_balance
        ...
        self.logic_side = target_side_int # 更新意图，等待下一轮循环处理

        # 状态 A：当前有持仓，且方向与目标相反 -> 只发平仓单
        if abs(physical_pos) > 1e-6:
            current_side = 1 if physical_pos > 0 else -1
            if current_side != target_side_int:
                print(f"[FLIP-STEP-1] 物理位置反转，发送平仓单。触发源: {source}")
                # 发送平仓市价单
                self._fire_atomic_order(
                    "SELL" if current_side == 1 else "BUY",
                    abs(physical_pos),
                    "MARKET",
                    "CLOSE_FOR_FLIP"
                )
                self.trigger_log.append((self.get_current_physical_timestamp_ns(), 0.0, -current_side, self.last_trade_price, source)) # 记录平仓动作及触发源
                return # 注意：此处直接 return，依靠 logic_loop 的下一轮循环处理开仓
        ...
        if abs(physical_pos) < 1e-6:
            ...
            if self.last_trade_price <= 0:
                ...
                self.trigger_log.append({"timestamp": self.get_current_physical_timestamp_ns(), "momentum": 0.0, "action": "NO_TRADE_PRICE_ZERO", "source": source}) # 记录触发源
                return
            target_qty_coin = Genes.UNIT_SIZE_USDT / self.last_trade_price
            
            print(f"[FLIP-STEP-2] 当前空仓，发送目标开仓单。方向: {target_side_str}")
            self._fire_atomic_order(target_side_str, target_qty_coin, "MARKET", "OPEN_POWER")
            self.trigger_log.append((self.get_current_physical_timestamp_ns(), 0.0, target_side_int, self.last_trade_price, source)) # 记录开仓动作及触发源
    ...
    def _dump_trigger_history(self):
        ...
        try:
            with open(filename, 'w', newline='') as f:
                writer = csv.writer(f)
                writer.writerow(["Timestamp_ns", "Momentum", "Side", "Price", "Source"]) # 增加 Source 列
                for entry in self.trigger_log:
                    # 如果 entry 是一个字典，则按照新的列顺序提取值，否则保持原样
                    if isinstance(entry, dict):
                        writer.writerow([
                            ...
                            entry.get("side"),
                            entry.get("price"),
                            entry.get("source", "UNKNOWN") # 提供默认值以防万一
                        ])
                    else:
                        # 对于旧格式的元组，假设它只有四个元素，并且没有 source
                        if len(entry) == 4:
                            writer.writerow([entry[0], entry[1], entry[2], entry[3], "UNKNOWN"])
                        else:
