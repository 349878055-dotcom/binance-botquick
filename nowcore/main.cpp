...
int main() {
    std::cout << "Size of OrderEventFrame: " << sizeof(OrderEventFrame) << " bytes" << std::endl;
    // 1. 注册退出信号
    ...
    // g_master_bridge->account_feed.bnb_balance.store(0.0, std::memory_order_release); // 已移除
    g_master_bridge->account_feed.position_amt.store(0.0, std::memory_order_release);
    // g_master_bridge->account_feed.avg_price.store(0.0, std::memory_order_release); // 已移除
