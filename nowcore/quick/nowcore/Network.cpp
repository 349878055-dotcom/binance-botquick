...
void init() { // Network 模块的初始化函数
    ...
    // 启用 CURL 详细调试信息
    curl_easy_setopt(g_curl_handle, CURLOPT_VERBOSE, 0L); // 在生产环境关闭详细模式
...
static ParsedMarketData parse_market_data_json_no_alloc(const char* msg, size_t len) {
    ...
    // 处理最优买卖盘消息，记录买一价、买一量、卖一价、卖一量
        ...
        if (strstr(msg, "\"u\":")) { // bookTicker 消息通常包含 "u": (更新ID) 字段
            ...
            const char* b_ptr = strstr(msg, "\"b\":\""); // 查找买一价
            if (b_ptr) frame.bid_p = strtod(b_ptr + 5, nullptr);
            const char* B_ptr = strstr(msg, "\"B\":\""); // 查找买一量
            if (B_ptr) frame.bid_q = strtod(B_ptr + 5, nullptr);

            const char* a_ptr = strstr(msg, "\"a\":\""); // 查找卖一价
            if (a_ptr) frame.ask_p = strtod(a_ptr + 5, nullptr);
            const char* A_ptr_actual = strstr(msg, "\"A\":\""); // 查找卖一量
            if (A_ptr_actual) frame.ask_q = strtod(A_ptr_actual + 5, nullptr);
                            ...
                            if (opcode == 0x9) { // 如果是 PING 帧
                                ...
                                // 路径 B: 用户私有流 (User Stream)
                                    ...
                                    if (strstr(msg_start, "\"e\":\"ORDER_TRADE_UPDATE\"")) { // 如果是订单交易更新事件
                                        const char* o_obj = strstr(msg_start, "\"o\":"); // 查找 "\"o\":" 字段，指向订单对象
                                        if (o_obj) { // 如果找到了订单对象

                                            // 1. 提取所有关键数据
                                            const char* client_order_id_ptr = strstr(o_obj, "\"c\":\"");
                                            char client_order_id[64] = {0}; // Increased buffer size for safety
                                            if (client_order_id_ptr) {
                                                const char* end_ptr = strchr(client_order_id_ptr + 5, '"');
                                                if (end_ptr) {
                                                    strncpy(client_order_id, client_order_id_ptr + 5, std::min((size_t)63, (size_t)(end_ptr - (client_order_id_ptr + 5))));
                                                }
                                            }

                                            const char* exch_order_id_ptr = strstr(o_obj, "\"i\":");
                                            char exch_order_id[64] = {0}; // Increased buffer size for safety
                                            if (exch_order_id_ptr) {
                                                // Find the end of the numeric ID, which is typically a comma or closing brace
                                                const char* end_ptr = strchr(exch_order_id_ptr + 4, ','); // Assuming "i":12345,
                                                if (!end_ptr) { // If not comma, try finding closing brace
                                                    end_ptr = strchr(exch_order_id_ptr + 4, '}');
                                                }
                                                if (end_ptr) {
                                                    strncpy(exch_order_id, exch_order_id_ptr + 4, std::min((size_t)63, (size_t)(end_ptr - (exch_order_id_ptr + 4))));
                                                }
                                            }
                                            
                                            const char* status_ptr = strstr(o_obj, "\"X\":\"");
                                            int order_status_event = EVT_NONE; // Default to NONE
                                            if (status_ptr) {
                                            ...
                                            const char* L_ptr = strstr(o_obj, "\"L\":\""); // Last trade price
                                            double fill_px = L_ptr ? strtod(L_ptr + 5, nullptr) : 0.0;
                                            const char* l_ptr = strstr(o_obj, "\"l\":\""); // Last trade quantity
                                            double fill_qty = l_ptr ? strtod(l_ptr + 5, nullptr) : 0.0;
                                            const char* q_ptr = strstr(o_obj, "\"q\":\""); // Original quantity
                                            double original_qty = q_ptr ? strtod(q_ptr + 5, nullptr) : 0.0;
                                            const char* z_ptr = strstr(o_obj, "\"z\":"); // Cumulative filled quantity (executedQty)
                                            double cum_fill_qty = z_ptr ? strtod(z_ptr + 4, nullptr) : 0.0; // "z" field is numeric, not string

                                            // Remaining quantity should be original_qty - cum_fill_qty
                                            double remaining_qty = original_qty - cum_fill_qty;
                                            if (remaining_qty < 0) remaining_qty = 0; // Ensure non-negative

                                            // 2. 核心触发 - 直接调用 Strategy::on_order_filled
                                            // Strategy 层将负责处理状态机去重和更新 g_master_bridge->account 状态
                                            Strategy::on_order_filled(client_order_id, exch_order_id, fill_px, fill_qty, remaining_qty, order_status_event);
