...
namespace Executor { // 定义 Executor 命名空间，封装所有交易执行相关功能
    ...
    void place_limit_order(const std::string& symbol, const std::string& side, double price, double quantity) {
        ...
        // 调用异步内部发单函数，订单类型为限价单 (ORD_LIMIT)，时间有效方式为 GTC (Good Till Cancel)。
        async_send_order_internal(client_order_id.c_str(), symbol, side, ORD_LIMIT, TIF_GTC, price, quantity);
    }
