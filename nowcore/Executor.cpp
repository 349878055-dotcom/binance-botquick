...
namespace Executor { // 定义 Executor 命名空间，封装所有交易执行相关功能
    ...
    void async_send_order_internal(const char* client_order_id, const std::string& symbol, const std::string& side, int order_type, int tif, double price, double quantity) {
        ...
        std::thread([=]() {
            ...
            if (!curl) {
                ...
                Strategy::on_order_filled(client_order_id, "", 0.0, 0.0, quantity, EVT_REJECTED); // 无法发单，直接拒绝
                return;
            }
            ...
            if (code == 200) { // 如果 HTTP 请求成功 (HTTP 状态码 200)
                ...
                Strategy::on_order_filled(client_order_id, exch_order_id_str.c_str(), fill_px, fill_qty, remaining_qty, event_type);
                ...
                // 通知 Strategy 模块订单被拒绝
                Strategy::on_order_filled(client_order_id, "", 0.0, 0.0, quantity, EVT_REJECTED);
     ...
     * @param symbol 交易对符号。
     */
    void cancel_order(const char* client_order_id, const std::string& symbol) {
        std::thread([=]() {
            CURL* curl = curl_easy_init(); // 每个线程拥有自己的 CURL 句柄
            if (!curl) {
                std::cerr << "[Executor][FATAL] Could not initialize CURL in cancel_order" << std::endl;
                Strategy::on_order_filled(client_order_id, "", 0.0, 0.0, 0.0, EVT_REJECTED); // 无法撤单，直接拒绝
                return;
            }
            // 配置新创建的 CURL 句柄，复制 Network::init() 中的设置
            curl_easy_setopt(curl, CURLOPT_TCP_NODELAY, 1L);
            ...
            if (code == 200) {
                ...
                // ... (简化处理，实际需要解析订单状态)
                Strategy::on_order_filled(client_order_id, "", 0.0, 0.0, 0.0, EVT_CANCELED);
            } else {
                std::cerr << "[Executor] 撤单失败 (Single Order) for " << symbol << ", client_order_id: " << cid_str << ": " << response << std::endl;
                Strategy::on_order_filled(client_order_id, "", 0.0, 0.0, 0.0, EVT_REJECTED);
            }
            curl_easy_cleanup(curl); // 清理本地 CURL 句柄
        }).detach();
    }
     ...
     */
    void amend_order(const char* client_order_id, const std::string& symbol, double new_price, double new_quantity) {
        std::thread([=]() {
            CURL* curl = curl_easy_init(); // 每个线程拥有自己的 CURL 句柄
            if (!curl) {
                std::cerr << "[Executor][FATAL] Could not initialize CURL in amend_order" << std::endl;
                Strategy::on_order_filled(client_order_id, "", 0.0, 0.0, 0.0, EVT_REJECTED); // 无法改单，直接拒绝
                return;
            }
            // 配置新创建的 CURL 句柄，复制 Network::init() 中的设置
            curl_easy_setopt(curl, CURLOPT_TCP_NODELAY, 1L);
            ...
            if (code == 200) {
                ...
                // 解析响应并触发 on_order_filled 事件
                // ... (简化处理，实际需要解析订单状态)
                Strategy::on_order_filled(client_order_id, "", new_price, new_quantity, 0.0, EVT_AMENDED);
            } else {
