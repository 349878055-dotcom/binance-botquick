...
namespace Executor { // 定义 Executor 命名空间，封装所有交易执行相关功能
    ...
    /**
     * @brief 辅助函数：从 JSON 字符串中提取指定键的 uint64_t 类型数值。
     * @param json_str 包含 JSON 数据的字符串。
     * @param key 要查找的 JSON 键。
     * @return 如果找到键并成功转换，返回对应的 uint64_t 值；否则返回 0ULL。
     */
    static uint64_t get_json_uint64(const std::string& json_str, const std::string& key) {
        std::string search_key = "\"" + key + "\":";
        const char* key_ptr = strstr(json_str.c_str(), search_key.c_str());
        if (key_ptr) {
            return std::strtoull(key_ptr + search_key.length(), nullptr, 10);
        }
        return 0ULL;
    }
     ...
     * @param parent_order_id 父订单 ID，用于平仓单追踪。
     ...
     * @param quantity 订单数量。
     * @param trigger_ms 交易所行情时间戳，作为订单物理链路起点。
     */
    void async_send_order_internal(const char* client_order_id, const char* parent_order_id, const std::string& symbol, const std::string& side, int order_type, int tif, double price, double quantity, uint64_t trigger_ms) {
        // 在一个新线程中执行异步操作，并将所有参数按值捕获（[=]）
        std::thread([=]() {
            CURL* curl = curl_easy_init(); // 每个线程拥有自己的 CURL 句柄
            if (!curl) {
                std::cerr << "[Executor][FATAL] Could not initialize CURL in async_send_order_internal" << std::endl;
                // 通知 Strategy 模块订单被拒绝，同时传递起点时间戳
                Strategy::on_order_filled(client_order_id, parent_order_id, "", 0.0, 0.0, quantity, EVT_REJECTED, 0ULL, false, trigger_ms); 
                return;
            }
            // 配置新创建的 CURL 句柄，复制 Network::init() 中的设置
            ...
            // curl_easy_setopt(curl, CURLOPT_DEBUGFUNCTION, Network::debug_callback); // 如果需要调试信息，注意 debug_callback 需要是 public
            ...
            // 获取并应用价格和数量精度
            int price_prec = 8; // 默认精度
            int quantity_prec = 8; // 默认精度

            if (g_master_bridge) {
                price_prec = g_master_bridge->price_precision.load(std::memory_order_acquire);
                quantity_prec = g_master_bridge->quantity_precision.load(std::memory_order_acquire);
                // 确保精度值在合理范围内，例如 0 到 10
                if (price_prec < 0 || price_prec > 10) price_prec = 8;
                if (quantity_prec < 0 || quantity_prec > 10) quantity_prec = 8;
            } else {
                std::cerr << "[Executor][WARNING] Shared memory bridge is null. Using default precision." << std::endl;
            }

            std::stringstream ss_quantity;
            ss_quantity << std::fixed << std::setprecision(quantity_prec) << quantity;
            params["quantity"] = ss_quantity.str(); // 设置订单数量，并应用数量精度

            std::string type_str; // 订单类型字符串
            
            if (order_type == ORD_MARKET) { // 如果是市价单
                type_str = "MARKET";
            } else { // 强制设置为市价单，废弃限价单逻辑
                type_str = "MARKET";
                // 限价单相关的价格和 TIF 参数不再需要设置
                // std::stringstream ss_price;
                // ss_price << std::fixed << std::setprecision(price_prec) << price;
                // params["price"] = ss_price.str(); // 限价单需要指定价格，并应用价格精度
            ...
            // 调用 Network 模块发送币安 API 请求，使用本地句柄
            ...
            if (code == 200) { // 如果 HTTP 请求成功 (HTTP 状态码 200)
                ...
                uint64_t transact_time = 0ULL; // 【新增】交易所成交时间 (纳秒)，作为订单物理链路终点
                ...
                if (order_status_str == "FILLED" || order_status_str == "PARTIALLY_FILLED") {
                    ...
                    if (avg_price_ptr) {
                        fill_px = std::strtod(avg_price_ptr + 12, nullptr); // 跳过 "\"avgPrice\":\""
                    }
                }

                // 【新增】解析 transactTime (交易所成交时间)，作为订单物理链路终点
                const char* transact_time_ptr = strstr(response.c_str(), "\"transactTime\":");
                if (transact_time_ptr) {
                    transact_time = std::strtoull(transact_time_ptr + 15, nullptr, 10) * 1000000ULL; // 转换为纳秒
                }
                ...
                const char* cum_fill_qty_ptr = strstr(response.c_str(), "\"cumFilledQty\":\"");
                if (cum_fill_qty_ptr) {
                    double cum_fill_qty = std::strtod(cum_fill_qty_ptr + 15, nullptr);
                    remaining_qty = quantity - cum_fill_qty;
                } else {
                    remaining_qty = quantity - fill_qty; // 如果没有 cumFilledQty，则近似计算
                }
                if (remaining_qty < 0) remaining_qty = 0; // 确保剩余数量不为负

                // 订单更新ID 和 is_maker (从下单响应中获取这两个字段可能不太准确，暂时设为默认值)
                // Binance 在下单响应中通常不直接提供这两个字段，它们更多出现在用户数据流中。
                // 暂时设置为 0ULL 和 false，后续可以根据实际 API 响应调整解析逻辑。
                uint64_t update_id = 0ULL; 
                bool is_maker = false; 

                // 通知 Strategy 模块有订单事件发生。Strategy 层负责去重和更新共享内存中的账户状态。
                // C++ 侧只负责将事件写入共享内存，Python 侧消费共享内存后才进行日志打印。
                Strategy::on_order_filled(client_order_id, parent_order_id, exch_order_id_str.c_str(), fill_px, fill_qty, remaining_qty, event_type, transact_time, update_id, is_maker, trigger_ms);
            ...
            curl_easy_cleanup(curl); // 清理本地 CURL 句柄
    ...
    /**
     * @brief 发送一个限价订单到交易所。 -- 【注释】不再使用限价单
     *        此函数会生成一个唯一的客户端订单 ID，并调用内部的异步发单函数。
     * @param client_order_id 客户端订单 ID。
     * @param parent_order_id 父订单 ID，用于平仓单追踪。
     * @param symbol 交易对符号。
     * @param side 订单方向（"BUY" 或 "SELL"）。
     * @param price 订单价格。
    ...
    //     async_send_order_internal(client_order_id, parent_order_id, symbol, side, ORD_LIMIT, tif, price, quantity, trigger_ms);
    // }
    ...
    void place_market_order(const char* client_order_id, const char* parent_order_id, const std::string& symbol, const std::string& side, double quantity, uint64_t trigger_ms) {
        ...
        async_send_order_internal(client_order_id, parent_order_id, symbol, side, ORD_MARKET, TIF_GTC, 0.0, quantity, trigger_ms);
    }
     ...
     * @param symbol 交易对符号。
     */
    // void cancel_all_orders(const std::string& symbol) { // 【删除】因为系统只使用市价单，无挂单可撤
    ...
    /**
     * @brief 撤销指定交易对的单笔挂单。 -- 【注释】不再使用单笔撤单
     * @param client_order_id 客户端订单 ID。
     * @param parent_order_id 父订单 ID，用于平仓单追踪。
     * @param symbol 交易对符号。
     * @param trigger_ms 交易所行情时间戳，作为订单物理链路起点。
    ...
    //         CURL* curl = curl_easy_init(); // 每个线程拥有自己的 CURL 句柄
    //         if (!curl) {
    //             std::cerr << "[Executor][FATAL] Could not initialize CURL in cancel_order" << std::endl;
    //             Strategy::on_order_filled(client_order_id, parent_order_id, "", 0.0, 0.0, 0.0, EVT_REJECTED, 0ULL, false, trigger_ms); // 无法撤单，直接拒绝
    //             return;
    //         }
    //         // 配置新创建的 CURL 句柄，复制 Network::init() 中的设置
    //         curl_easy_setopt(curl, CURLOPT_TCP_NODELAY, 1L);
    ...
    //         std::map<std::string, std::string> params; // 请求参数
    //         params["symbol"] = symbol; // 设置交易对
    //         params["origClientOrderId"] = cid_str; // 设置要撤销的客户端订单 ID
    //
    //         std::string response; // 用于存储 API 响应的缓冲区
    //         long code = Network::perform_binance_request(
    //             curl, // 使用本地句柄
    //             "DELETE", // HTTP 方法为 DELETE
    //             "/fapi/v1/order", // 币安撤销单笔订单的 API 路径
    //             params,
    //             response,
    //             g_api_key,
    //             g_api_secret,
    ...
    //
    //         if (code == 200) {
    //             std::cout << "[Executor] 撤单成功 (Single Order) for " << symbol << ", client_order_id: " << cid_str << "." << std::endl;
    //             // 解析响应并触发 on_order_filled 事件
    //             // 【新增】解析 transactTime (交易所成交时间)
    //             uint64_t transact_time = get_json_uint64(response, "updateTime") * 1000000ULL; // 撤单接口返回的是 updateTime
    //             Strategy::on_order_filled(client_order_id, parent_order_id, "", 0.0, 0.0, 0.0, EVT_CANCELED, transact_time, 0ULL, false, trigger_ms);
    //         } else {
    //             std::cerr << "[Executor] 撤单失败 (Single Order) for " << symbol << ", client_order_id: " << cid_str << ": " << response << std::endl;
    //             Strategy::on_order_filled(client_order_id, parent_order_id, "", 0.0, 0.0, 0.0, EVT_REJECTED, 0ULL, false, trigger_ms);
    //         }
    //         curl_easy_cleanup(curl); // 清理本地 CURL 句柄
    //     }).detach();
    // }
    ...
    /**
     * @brief 修改指定交易对的挂单（价格和数量）。 -- 【注释】不再使用改单
     * @param client_order_id 客户端订单 ID。
     * @param parent_order_id 父订单 ID，用于平仓单追踪。
     * @param symbol 交易对符号。
    ...
    //         CURL* curl = curl_easy_init(); // 每个线程拥有自己的 CURL 句柄
    //         if (!curl) {
    //             std::cerr << "[Executor][FATAL] Could not initialize CURL in amend_order" << std::endl;
    //             Strategy::on_order_filled(client_order_id, parent_order_id, "", 0.0, 0.0, 0.0, EVT_REJECTED, 0ULL, false, trigger_ms); // 无法改单，直接拒绝
    //             return;
    //         }
    //         // 配置新创建的 CURL 句柄，复制 Network::init() 中的设置
    //         curl_easy_setopt(curl, CURLOPT_TCP_NODELAY, 1L);
    ...
    //         std::map<std::string, std::string> params; // 请求参数
    //         params["symbol"] = symbol; // 设置交易对
    //         params["origClientOrderId"] = cid_str; // 设置要修改的客户端订单 ID
    //         params["side"] = "NONE"; // 占位符，币安改单接口不直接支持 side 参数，但需要传递以通过通用函数
    //         params["price"] = std::to_string(new_price); // 新价格
    //         params["quantity"] = std::to_string(new_quantity); // 新数量
    //
    //         std::string response; // 用于存储 API 响应的缓冲区
    //         long code = Network::perform_binance_request(
    //             curl, // 使用本地句柄
    //             "PUT", // HTTP 方法为 PUT
    //             "/fapi/v1/order", // 币安修改订单的 API 路径
    //             params,
    //             response,
    //             g_api_key,
    //             g_api_secret,
    ...
    //
    //         if (code == 200) {
    //             std::cout << "[Executor] 改单成功 for " << symbol << ", client_order_id: " << cid_str << ", new_price: " << new_price << ", new_quantity: " << new_quantity << "." << std::endl;
    //             // 解析响应并触发 on_order_filled 事件
    //             // 【新增】解析 transactTime (交易所成交时间)
    //             uint64_t transact_time = get_json_uint64(response, "updateTime") * 1000000ULL; // 改单接口返回的是 updateTime
    //             Strategy::on_order_filled(client_order_id, parent_order_id, "", new_price, new_quantity, 0.0, EVT_AMENDED, transact_time, 0ULL, false, trigger_ms);
    //         } else {
    //             std::cerr << "[Executor] 改单失败 for " << symbol << ", client_order_id: " << cid_str << ": " << response << std::endl;
    //             Strategy::on_order_filled(client_order_id, parent_order_id, "", 0.0, 0.0, 0.0, EVT_REJECTED, 0ULL, false, trigger_ms);
    //         }
    //         curl_easy_cleanup(curl); // 清理本地 CURL 句柄
    //     }).detach();
    ...
    void fetch_account_info(const std::string& symbol, GenericShmStruct* shm_bridge) {
        ...
        if (http_code == 200) { // 如果 HTTP 请求成功
            ...
            // 原子地更新共享内存中的 USDT 余额
            shm_bridge->account_feed.usdt_balance.store(usdt_free_balance, std::memory_order_release);
            ...
            // 原子地更新共享内存中的 BNB 余额
            shm_bridge->account_feed.bnb_balance.store(bnb_free_balance, std::memory_order_release);
            ...
            if (pos_ptr) {
                ...
                if (amt_ptr) {
                    ...
                    shm_bridge->account_feed.position_amt.store(pos_amt, std::memory_order_release); // 原子地更新持仓量
                }
                const char* entry_ptr = strstr(pos_ptr, "\"entryPrice\":\"");
                if (entry_ptr) {
                    double entry_px = std::strtod(entry_ptr + 14, nullptr);
                    shm_bridge->account_feed.avg_price.store(entry_px, std::memory_order_release); // 原子地更新持仓均价
                }
            }
            ...
            std::cout << "[Executor] 成功获取账户信息。USDT 可用余额: " << usdt_free_balance 
                      ...
                      << ", 持仓量: " << shm_bridge->account_feed.position_amt.load() << ", 均价: " << shm_bridge->account_feed.avg_price.load() << std::endl;
        } else {
