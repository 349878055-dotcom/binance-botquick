...
namespace Executor { // 定义 Executor 命名空间，封装所有交易执行相关功能
     ...
     * @param quantity 订单数量。
     */
    void place_limit_order(const std::string& symbol, const std::string& side, double price, double quantity, int tif) {
        // 生成一个临时的客户端订单 ID，基于当前纳秒时间戳，确保唯一性。
        std::string client_order_id = "CLORD_" + std::to_string(Common::get_now_ns()); 
        // 调用异步内部发单函数，订单类型为限价单 (ORD_LIMIT)，现在把 tif 传进去。
        async_send_order_internal(client_order_id.c_str(), symbol, side, ORD_LIMIT, tif, price, quantity);
    }
    ...
    void place_market_order(const std::string& symbol, const std::string& side, double quantity) {
        ...
        async_send_order_internal(client_order_id.c_str(), symbol, side, ORD_MARKET, TIF_GTC, 0.0, quantity);
    }
