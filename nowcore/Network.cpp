...
void init() {
    ...
    const char* subscribe_aggtrade_payload = "{\"method\":\"SUBSCRIBE\",\"params\":[\"bnbusdt@aggTrade\"],\"id\":1}"; // 只订阅 aggTrade
    ...
    if (!g_listen_key.empty()) {
        ...
        if (g_user_ws_client.fd_ < 0) {
            ...
            // 获取交易对精度信息并写入共享内存
            if (g_master_bridge) {
                Executor::fetch_and_set_precision("BNBUSDT", g_master_bridge);
            } else {
            ...
            // [新增] 获取初始账户信息并写入共享内存
            if (g_master_bridge) {
                Executor::fetch_account_info("BNBUSDT", g_master_bridge);
            } else {
