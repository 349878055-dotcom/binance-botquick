    ...
    while (g_running) {
        ...
        if (g_master_bridge) {
            ...
            while (r_idx < w_idx) {
                ...
                const char* cid = frame.client_order_id; // [新增] 提取 ID
                const char* parent_cid = frame.parent_order_id; // [新增] 提取 Parent ID
                uint64_t trigger_ms = frame.trigger_ms; // 获取触发时间

                // 不判断 action，直接无条件执行一次 Executor::place_market_order
                Executor::place_market_order(cid, parent_cid, symbol, side_str, frame.quantity, trigger_ms);
