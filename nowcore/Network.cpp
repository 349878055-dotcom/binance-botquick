        ...
        if (g_master_bridge) {
            ...
            while (r_idx < w_idx) {
                ...
                if (frame.action == ACT_NEW) {
                    ...
                    //     Executor::place_limit_order(cid, symbol, side_str, frame.price, frame.quantity, frame.tif, trigger_ms);
                    // } 
                    // else if (frame.type == ORD_MARKET) {
                        Executor::place_market_order(cid, symbol, side_str, frame.quantity, trigger_ms); // 强制市价单
                    // }
                else if (frame.action == ACT_CANCEL_ALL) {
                    // 撤销所有
                    //printf("[Command] 收到全撤指令: %s\n", symbol.c_str());
                    Executor::cancel_all_orders(symbol); // cancel_all_orders 不接收 trigger_ms，因为是批量操作
                }
                // else if (frame.action == ACT_CANCEL) { // 【注释】不再处理单笔撤单
                //     // 单笔撤单
                //     //printf("[Command] 收到单撤指令: %s %s\n", symbol.c_str(), cid);
                //     Executor::cancel_order(cid, symbol, trigger_ms);
                // }
                // else if (frame.action == ACT_AMEND) { // 【注释】不再处理改单
                //     // 改单
                //     //printf("[Command] 收到改单指令: %s %s new_price=%.2f new_quantity=%.4f\n",
                //            //symbol.c_str(), cid, frame.new_price, frame.new_quantity);
                //     Executor::amend_order(cid, symbol, frame.new_price, frame.new_quantity, trigger_ms);
                // }
                                ...
                                // 路径 B: 用户私有流 (User Stream)
                                    ...
                                    if (strstr(msg_start, "\"e\":\"ORDER_TRADE_UPDATE\"")) {
                                        ...
                                        if (o_obj) {
                                            ...
                                            Strategy::on_order_update_private_stream(client_order_id, exch_order_id, fill_px, fill_qty, remaining_qty, order_status_event, update_id, is_maker, transact_time, 0ULL); 
