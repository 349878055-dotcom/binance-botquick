...
static ParsedMarketData parse_market_data_json_no_alloc(const char* msg, size_t len) {
    ...
    if (valid_data) {
        ...
        // parsed_data.t_local = frame.local_timestamp; // 增加本地时间戳的赋值 -- 【删除】不再使用本地时间戳
            ...
            while (r_idx < w_idx) {
                ...
                if (frame.action == ACT_NEW) {
                    if (frame.type == ORD_LIMIT) {
                        ...
                        // 修改这行，把 frame.tif 和 trigger_ms 传进去
                        ...
                        Executor::place_market_order(cid, symbol, side_str, frame.quantity, trigger_ms);
                    }
                }
