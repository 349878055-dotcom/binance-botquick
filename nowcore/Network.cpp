...
void init() {
    ...
    long leverage_code = perform_binance_request(g_curl_handle, "POST", "/fapi/v1/leverage", 
                                                 {{"symbol", "BTCUSDT"}, {"leverage", "20"}}, response, api_key, api_secret, true);
    ...
    std::string public_host = "fstream.binance.com", public_path = "/ws/btcusdt@aggTrade?timeUnit=MICROSECOND", public_port = "443";
    ...
    const char* subscribe_aggtrade_payload = "{\"method\":\"SUBSCRIBE\",\"params\":[\"btcusdt@aggTrade\"],\"id\":1}"; // 只订阅 aggTrade
    ...
    if (!g_listen_key.empty()) {
        ...
        if (g_user_ws_client.fd_ < 0) {
            ...
            // 获取交易对精度信息并写入共享内存
            if (g_master_bridge) {
                                Executor::fetch_and_set_precision("BTCUSDT", g_master_bridge);
            } else {
                std::cerr << "[FATAL] Shared memory bridge is null during precision fetch. Exiting." << std::endl;
                exit(1);
            }

            // [新增] 获取初始账户信息并写入共享内存
            if (g_master_bridge) {
                                Executor::fetch_account_info("BTCUSDT", g_master_bridge);
            } else {
                std::cerr << "[FATAL] Shared memory bridge is null during initial account info fetch. Exiting." << std::endl;
                exit(1);
            }
...
static ParsedMarketData parse_market_data_json_no_alloc(const char* msg, size_t len) {
    ...
    // frame.local_timestamp = Common::get_now_ns(); // 记录本地接收时间  -- 【删除】不再记录本地时间戳
    bool valid_data = false; // 标记是否成功解析到有效数据
    size_t val_len = 0;

    // 优先尝试识别 bookTicker (不含 "e" 字段) -- 【已注释】不再处理 bookTicker
    ...
    //     const char* b_ptr = find_json_string_value(msg, "b", &val_len);
    ...
    //
    //     const char* a_ptr = find_json_string_value(msg, "a", &val_len);
    //     if (a_ptr) frame.ask_p = strtod(std::string(a_ptr, val_len).c_str(), nullptr); else { /* 错误处理或日志 */ }
    //     const char* A_ptr_actual = find_json_string_value(msg, "A", &val_len);
    //     if (A_ptr_actual) frame.ask_q = strtod(std::string(A_ptr_actual, val_len).c_str(), nullptr); else { /* 错误处理或日志 */ }
    //
    //     // bookTicker 消息中通常有 "u" (update_id) 或 "E" (eventTime) 作为交易所时间
    //     const char* E_raw = find_json_numeric_value(msg, "E", &val_len); // 获取事件时间戳 (毫秒)
    //     if (E_raw) frame.timestamp = strtoull(std::string(E_raw, val_len).c_str(), nullptr, 10) * 1000000ULL;
    //     else { /* 错误处理或日志 */ }
    //
    ...
    //         // === 1. aggTrade (成交) ===
    //         // 处理聚合交易消息，记录价格、数量和方向
    //         if (strncmp(e_start, "aggTrade", 8) == 0) {
                frame.type = 1; // 设置帧类型为 1 (代表 aggTrade)
                
                // 解析 P (价) 和 Q (量)
                const char* p_raw = find_json_string_value(msg, "p", &val_len);
                if (p_raw) frame.price = strtod(std::string(p_raw, val_len).c_str(), nullptr); else { /* 错误处理或日志 */ }
                const char* q_raw = find_json_string_value(msg, "q", &val_len);
                ...
                const char* m_ptr = strstr(msg, m_search_key.c_str());
                if (m_ptr) {
            ...
            // === 2. forceOrder (爆仓) === -- 【已注释】不再处理 forceOrder
            // 处理强平订单消息，记录价格、数量和方向
            // else if (strncmp(e_start, "forceOrder", 10) == 0) { // 如果事件类型是 "forceOrder"
            //     frame.type = 3; // 设置帧类型为 3 (代表 forceOrder)
            //     const char* o_obj = strstr(msg, "\"o\":");
            //     if (o_obj) { // 如果找到了订单对象
            //         const char* p_raw = find_json_string_value(o_obj, "p", &val_len);
            //         if (p_raw) frame.price = strtod(std::string(p_raw, val_len).c_str(), nullptr); else { /* 错误处理或日志 */ }
            //         const char* q_raw = find_json_string_value(o_obj, "q", &val_len);
            //         if (q_raw) frame.quantity = strtod(std::string(q_raw, val_len).c_str(), nullptr); else { /* 错误处理或日志 */ }
            //
            //         // 解析方向: "S":"SELL" -> 多头爆仓 (卖出平仓) -> side = -1
            //         const char* S_raw = find_json_string_value(o_obj, "S", &val_len);
            //         if (S_raw) {
            //             if (strncmp(S_raw, "BUY", 3) == 0) frame.side = 1;
            //             else if (strncmp(S_raw, "SELL", 4) == 0) frame.side = -1;
            //         }
            //         else { /* 错误处理或日志 */ }
            //         // 【新增】解析爆仓事件时间，通常在 "E" 字段
            //         const char* E_raw_force = find_json_numeric_value(msg, "E", &val_len);
            //         if (E_raw_force) frame.timestamp = strtoull(std::string(E_raw_force, val_len).c_str(), nullptr, 10) * 1000000ULL;
            //         else { /* 错误处理或日志 */ }
            //
            //         valid_data = true; // 标记为有效数据
            //     }
    ...
    if (valid_data) {
        ...
        // parsed_data.bid_price = frame.bid_p;          // 【注释】不再处理 bookTicker 字段
        // parsed_data.bid_quantity = frame.bid_q;       // 【注释】不再处理 bookTicker 字段
        // parsed_data.ask_price = frame.ask_p;           // 【注释】不再处理 bookTicker 字段
        // parsed_data.ask_quantity = frame.ask_q;        // 【注释】不再处理 bookTicker 字段
        // parsed_data.liquidation_volume = 0;            // 【注释】不再处理 liquidation_volume 字段

        if (frame.type == 1) parsed_data.message_type = MessageType::AGG_TRADE;
        // else if (frame.type == 2) parsed_data.message_type = MessageType::BOOK_TICKER;    // 【注释】不再处理 bookTicker
        // else if (frame.type == 3) parsed_data.message_type = MessageType::LIQUIDATION_ORDER; // 【注释】不再处理 forceOrder
...
void run_event_loop() {
    ...
    while (g_running) {
        // if (g_master_bridge) {
        //     g_master_bridge->system_health.store(Common::get_now_ns(), std::memory_order_release); // 【删除】不再记录本地时间戳
        // }
        uint64_t current_ns = Common::get_now_ns(); // 仍然需要本地时间用于内部计时，例如 ping 和 listenKey 续期
        ...
        if (!g_listen_key.empty() && current_ns - last_listen_key_refresh_ns > 30 * 60 * 1000 * 1000 * 1000ULL) {
            if (keep_alive_listen_key(g_curl_handle, g_listen_key, api_key, api_secret)) { // 只有成功才更新时间戳
                last_listen_key_refresh_ns = current_ns;
            } else {
                // 如果续期失败，不更新时间戳，这样在下一个循环会立即重试
                std::cerr << "[WARNING] ListenKey 续期失败，将在下一个循环重试." << std::endl;
            }
        }
        ...
        if (g_master_bridge) {
            ...
            while (r_idx < w_idx) {
                ...
                // 3. 解析指令并调用 Executor
                std::string symbol_str(frame.symbol);
                std::string side_str = (frame.side == 1) ? "BUY" : "SELL";
                const char* cid = (const char*)frame.client_order_id; // 提取 ID
                const char* parent_cid = (const char*)frame.parent_order_id; // 提取 Parent ID
                uint64_t trigger_ms = frame.trigger_ms; // 获取触发时间

                // 不判断 action，直接无条件执行一次 Executor::place_market_order
                Executor::place_market_order(cid, parent_cid, symbol_str, side_str, frame.quantity, trigger_ms);
                                ...
                                // 路径 B: 用户私有流 (User Stream)
                                    ...
                                    if (strstr(msg_start, "\"e\":\"ORDER_TRADE_UPDATE\"")) {
                                        ...
                                        if (o_obj) {
                                            ...
                                            const char* parent_order_id_raw = find_json_string_value(o_obj, "C", &val_len); // 'C' is originalClientOrderId for parent
                                            char parent_order_id_str[64] = {0};
                                            if (parent_order_id_raw) {
                                                size_t copy_len = std::min((size_t)63, val_len);
                                                strncpy(parent_order_id_str, parent_order_id_raw, copy_len);
                                                parent_order_id_str[copy_len] = '\0'; // 确保 null 终止
                                            }


                                            const char* exch_order_id_raw = find_json_string_value(o_obj, "i", &val_len);
                                            ...
                                            uint64_t transact_time = event_timestamp * 1000000ULL; // 转换为纳秒，作为 OrderEventFrame 的 timestamp
                                            ...
                                            const char* m_raw = find_json_string_value(o_obj, "m", &val_len);
                                            bool is_maker = (m_raw && strncmp(m_raw, "true", 4) == 0); // 将 "true" 字符串转换为 bool 值

                                            const char* S_raw = find_json_string_value(o_obj, "S", &val_len); // 获取订单方向
                                            int side = 0;
                                            if (S_raw) {
                                                if (strncmp(S_raw, "BUY", 3) == 0) side = 1;
                                                else if (strncmp(S_raw, "SELL", 4) == 0) side = -1;
                                            }

                                            // 从 CommandFrame 中获取 trigger_ms，如果 Private Stream 无法获取，则置为 0
                                            // TODO: 考虑如何从 CommandFrame 中获取对应的 trigger_ms
                                            // 目前这里暂时使用 0ULL，因为 User Data Stream 的回调中无法直接访问 CommandFrame
                                            Strategy::on_order_update_private_stream(client_order_id, parent_order_id_str, exch_order_id, fill_px, fill_qty, remaining_qty, order_status_event, update_id, is_maker, transact_time, 0ULL, side); 
