            ...
            while (r_idx < w_idx) {
                ...
                if (frame.action == ACT_NEW) {
                    if (frame.type == ORD_LIMIT) {
                        ...
                        Executor::place_limit_order(cid, symbol, side_str, frame.price, frame.quantity, frame.tif);
                    } 
                    else if (frame.type == ORD_MARKET) {
                        // 市价单
                        //printf("[Command] 收到市价单: %s %s %.4f\n", 
                               symbol.c_str(), side_str.c_str(), frame.quantity);
                        Executor::place_market_order(cid, symbol, side_str, frame.quantity);
                    }
                }
                    ...
                    Executor::cancel_order(cid, symbol);
                }
                    ...
                    Executor::amend_order(cid, symbol, frame.new_price, frame.new_quantity);
                }
