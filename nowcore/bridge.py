...
class Bridge:
    ...
    def fetch_market_stream(self):
        ...
        if remote_write_idx - self.local_market_read_idx > RING_BUFFER_SIZE:
            print(f"[ERROR] MarketRingBuffer 发生溢出！Python 消费速度跟不上 C++ 生产速度。")
            print(f"[ERROR] 丢失了 {remote_write_idx - self.local_market_read_idx - RING_BUFFER_SIZE} 条数据。")
            print(f"[ERROR] 将从最新的数据开始追赶，之前的旧数据已被覆盖。")
            self.local_market_read_idx = remote_write_idx # 直接跳到最新的写入位置
            return # 停止当前批次的消费，等待下一轮，或者可以重新进入循环
    ...
    def fetch_account_stream(self):
        ...
        if remote_write_idx - self.local_account_read_idx > EVENT_CAPACITY:
            print(f"[ERROR] AccountRingBuffer 发生溢出！Python 消费速度跟不上 C++ 生产速度。")
            print(f"[ERROR] 丢失了 {remote_write_idx - self.local_account_read_idx - EVENT_CAPACITY} 条订单事件数据。")
            print(f"[ERROR] 将从最新的数据开始追赶，之前的旧数据已被覆盖。")
    ...
    def send_limit_order(self, symbol, side, price, quantity, tif_type=3): 
        """
        发送限价单
        ...
        if (w_idx - r_idx) >= COMMAND_CAPACITY:
            print(f"[ERROR] CommandRingBuffer 已满！无法发送新指令。")
            print(f"[ERROR] 当前写入索引: {w_idx}, 读取索引: {r_idx}, 容量: {COMMAND_CAPACITY}")
            return None # 无法发送指令，返回 None 或抛出异常
