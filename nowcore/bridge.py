...
class MarketFrame(ctypes.Structure):
    ...
    _fields_ = [
        ...
        ("bid_p",           ctypes.c_double),
        ("ask_p",           ctypes.c_double),
        ("quantity",        ctypes.c_double),
        ("bid_q",           ctypes.c_double),
        ("ask_q",           ctypes.c_double),
        ("type",            ctypes.c_int),
        ("side",            ctypes.c_int),
    ]
...
class Bridge:
    ...
    def send_limit_order(self, symbol, side, price, quantity, market_timestamp_start, tif_type, parent_order_id=""): 
        """
        发送限价单
